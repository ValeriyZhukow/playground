angular.module('nccCollaborations.itemsView').
    controller('bulkRequestExtensionModalCtrl', function ($scope, $modalInstance, $filter, items, datesService) {

        var items_with_expire = _.filter(items,
            function(item){ return item.expire; });
        if (items_with_expire.length) {
            $scope.item = items_with_expire[0];
        } else {
            $scope.item = items[0];
        }
