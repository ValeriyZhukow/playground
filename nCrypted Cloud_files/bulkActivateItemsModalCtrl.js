angular.module('nccCollaborations.itemsView').
    controller('bulkActivateItemsModalCtrl', function ($scope, $modalInstance, items, datesService, itemsService) {

        $scope.hasExpiredItems = itemsService.hasExpiredSecureViews(items);
        ...
        if ( $scope.hasExpiredItems ) {
            ...
            $scope.organizationNameLabel = $scope.item.collaborations[0].identity.organization;

            $scope.differentMaxExpirations = itemsService.hasDifferentMaxExpirations(items);

            $scope.expiredItemsCountLabel = itemsService.getExpiredSecureViews( items ).length;
            ...
            $scope.activateItemsWithExpireDate = function() {
                $modalInstance.close( $scope.selectedExtendDuration.toISOString() );
            };
